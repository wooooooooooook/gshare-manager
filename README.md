...
- Home Assistant MQTT Discovery로 상태 센서, `manual_sync_android_on` 버튼, `latest_screenshot` 카메라 엔티티 제공(최근 스크린샷을 MQTT로 발행)
- VM CPU 사용량 모니터링하여 Idle상태 시 자동 종료 웹훅 전송 및 SMB 공유 중단
- VM 내부 `/api/vm-stop` 호출은 15분 동안 종료 신호가 연속 수신될 때만 VM 종료를 실행하며(신호 간 공백 5분 이상이면 누적 초기화), 종료 시각 기록 및 SMB 공유 중단 워크플로우 수행
- 상태 모니터링 웹 인터페이스

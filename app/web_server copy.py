...
def init_server(manager, app_config: GshareConfig):
    """웹서버 초기화"""
    try:
        logging.debug("웹 서버 상태 초기화 시작")
        global gshare_manager, config, is_setup_complete
...
def main_page():
    ...
    # 기본 대시보드 표시
    if gshare_manager:
        # gshare_manager.current_state 직접 참조
        return render_template('index.html', state=gshare_manager.current_state, config=config)
    else:
...
def update_state():
    global gshare_manager
    
    # 상태 확인 및 로깅
    try:
        if gshare_manager is not None and hasattr(gshare_manager, 'current_state'):
            # gshare_manager 객체에서 직접 상태 참조
            if gshare_manager.current_state:
                return jsonify(gshare_manager.current_state.to_dict())
            else:
...
def start_vm():
    try:
        if gshare_manager is None:
            return jsonify({"status": "error", "message": "서버가 아직 초기화되지 않았습니다."}), 404

        if gshare_manager.current_state.vm_running:
...
def shutdown_vm():
    try:
        if gshare_manager is None or config is None:
            return jsonify({"status": "error", "message": "서버가 아직 초기화되지 않았습니다."}), 404

        if not gshare_manager.current_state.vm_running:
